...
I can do this by looking at the similarity of the ten recommendations generated for each user.
...
Next, I create a function to calculate the similarities of the items

```{r}
item_sim_function <- function(user_items) {
  similarities_matrix <- similarity(train_set_matrix[,user_items], method = 'cosine',which='items')

  mean(similarities_matrix)
}
```
...
```{r}
ubcf_pred_similarities <- sapply(ubcf_pred_list, item_sim_function)
ibcf_pred_similarities <- sapply(ibcf_pred_list, item_sim_function)
ubcf50_pred_similarities <- sapply(ubcf50_pred_list, item_sim_function)
ubcf100_pred_similarities <- sapply(ubcf100_pred_list, item_sim_function)
ubcf200_pred_similarities <- sapply(ubcf200_pred_list, item_sim_function)
ubcf300_pred_similarities <- sapply(ubcf300_pred_list, item_sim_function)
```

I then take the average of all the values to aggregate into one metric. I then add this metric to the metrics table.

```{r}
ubcf_similarity <- mean(ubcf_pred_similarities)
ibcf_similarity <- mean(ibcf_pred_similarities)
ubcf50_similarity <- mean(ubcf50_pred_similarities)
ubcf100_similarity <- mean(ubcf100_pred_similarities)
ubcf200_similarity <- mean(ubcf200_pred_similarities)
ubcf300_similarity <- mean(ubcf300_pred_similarities)
```
...
```{r}
similarity_values <- c(
  ubcf    = ubcf_similarity,
  ibcf    = ibcf_similarity,
  ubcf50  = ubcf50_similarity,
  ubcf100 = ubcf100_similarity,
  ubcf200 = ubcf200_similarity,
  ubcf300 = ubcf300_similarity
)

metrics_table <- cbind(
  metrics_table,
  similarity = similarity_values[rownames(metrics_table) ]
)
```
...
However, while the ubcf300 model had the best performance metrics, it had the worst diversity, as its similarity rating of recommendations was the highest. Conversely, the model with the worst accuracy performance metrics had the most diversity (ubcf with a similarity of 0.68).

This result is different from what I initially suspected (worse performance but better diversity with larger neighborhoods). It seems that the smaller neighborhoods were overfitting the data (high variance, low bias) and the tipping point were performance degrades with larger neighborhood sizes was not yet reached.

Interestingly, while ubcf300 had the worst diversity (nearly 25%, 1 minus the similarity), this is close to the 30% value that was mentioned in "Recommender systems: from algorithms to user experience" as the degree of moderate diversity that users prefer in their recommendation list. Thus, I would recommend the ubcf300 model as it had the best performance while still having a near optimal degree of diversity. However, one drawback of the recommenderlab approach to topN recommendations is that by converting it to a binary classifying problem (likes vs did not like), the recommender is not considering how much a user liked or disliked an item (i.e., it is treating extremely good and extremely bad recommendations the same).
